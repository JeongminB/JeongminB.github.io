...
확률변수 $X$의 기댓값을 $\mu$라고 합시다. $X$에서 샘플링한 $n$개의 표본에 대해서 구하는 표본평균 $\bar X$는 아래와 같이 구할 수 있습니다. 여기에는 $n$개의 표본을 각각 독립동일분포인 확률분포로 가정하여, 각 샘플이 나올 확률을 $\frac{1}{n}$로 동일하게 봅니다.
...
고로 표본분산을 불편 추정량으로 만들어주기 위해서는 편차제곱합을 $n-1$로 나누어줘야 합니다. 아직 표본분산을 구하기 위해 $n-1$로 나누는 이유가 와닿지 않는다면 <a href="/basics/statistics/bessel">다음 글</a>을 참고합시다.